...
拿到 `manifest` 后，**优先读取 `contact_sheets[]`**——每张是一段按时序拼好的分组图（每格标了时间戳/帧号，红框 + CUT 标记镜头切换点）。**每批并行读入 3–5 张**（在一条消息里发 3–5 个读取），既能并行提速，又不会因一屏塞太多图而漏看字幕/细节，保证分析精度；按时序一批批读完全部拼图（长视频可能 20 张+，就分多批），快速把握全片的镜头演变。据此逐镜分析场景、景别、构图、运镜趋势、人物动作、情绪、可见字幕/台词与节奏变化。**某段细节存疑时**，再按 `contact_sheets[].cells[].index` 到 `frame_extraction.frames[]` 找到对应原帧、读取放大确认（帧文件仍在 `frames/` 里）。读图方式从"逐张几十上百帧"降到"分批读拼图（每批 3–5 张）+ 少量定向关键帧"。不可见或未转写的声音/对白不要编造，只能标注为“画面不可确认”或基于字幕判断。
