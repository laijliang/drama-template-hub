...
拿到 `manifest` 后，**优先读取 `contact_sheets[]`**——每张是一段按时序拼好的分组图（每格标了时间戳/帧号，红框 + CUT 标记镜头切换点）。**每批并行读入 6–8 张**（在一条消息里发 6–8 个读取；超长片可到 10 张），按时序一批批读完全部拼图（长视频可能 20~40 张，就分多批），快速把握全片的镜头演变。每张拼图都是独立全分辨率读入的，同一批多几张**不会让画面变糊、不损分析精度**，只是多占些上下文——所以在上下文够用时优先加大批次，减少来回次数。据此逐镜分析场景、景别、构图、运镜趋势、人物动作、情绪、可见字幕/台词与节奏变化。**某段细节存疑时**，再按 `contact_sheets[].cells[].index` 到 `frame_extraction.frames[]` 找到对应原帧、读取放大确认（帧文件仍在 `frames/` 里）。读图方式从"逐张几十上百帧"降到"分批读拼图（每批 6–8 张）+ 少量定向关键帧"。不可见或未转写的声音/对白不要编造，只能标注为“画面不可确认”或基于字幕判断。
...
- [ ] 台词完全是新写的？ ✅
- [ ] 叙事视角与原作一致（第一人称/第三人称）？ ✅
- [ ] 每个分镜时长都在原片对应镜头基础上 +2~3 秒（逐镜加，不是整片加）？ ✅
- [ ] 分镜前后逻辑自洽（镜与镜衔接连贯、因果通顺、无跳切断裂）？ ✅
...
#### 【分镜时长：每镜各自 +2~3 秒 + 前后逻辑自洽】

复刻不是逐帧照抄剪好的原片，而是在**完全读懂原片剧情**的基础上，把被剪掉的合理细节补回来，让每一镜都从容、且镜与镜之间衔接自洽：

- **每个分镜的 `durationSeconds`，都在原片对应镜头时长的基础上「至少 +2~3 秒」**——这是**每一镜各自加**，不是把整片总时长加 2~3 秒。原片为卡节奏往往剪得很紧，复刻要给动作、表情、留白足够的呼吸空间。若原片某镜时长拿不准，就按该镜要演完的动作/台词估一个合理时长，再 +2~3 秒。
- **前后逻辑自洽（硬性要求）**：先把原片故事线**完整梳理清楚**再逐镜复刻；镜与镜之间的时间、地点、人物状态、动作与因果必须连贯——上一镜的动作/情绪要能顺到下一镜，不出现跳切断裂、前后矛盾、莫名其妙的转场或人物状态突变。
- **用补全的细节把加长的时长填实**：原片一闪而过或被剪掉的过渡动作、环境交代、情绪铺垫，在复刻里补成完整可绘画的镜头——既撑起「+2~3 秒」的时长，又把镜与镜的逻辑接顺。
- 补细节不等于注水：补的是"原本就该有、被剪掉"的内容，服务于叙事连贯，不是硬加与主线无关的镜头。
- `durationSeconds` 按「加长后」的时长填写。
...
8. **分镜必须标时长且逐镜加长**：`shots[]` 里每一镜都必须含 `durationSeconds`（数字，单位秒）；每镜时长在原片对应镜头基础上**逐镜 +2~3 秒**（非整片总时长加），并保证镜与镜**前后逻辑自洽**。详见阶段二「分镜时长」章节。
